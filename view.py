...
class View(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # edit_menu.add_command(label="Редактировать XML декларацию", command=self.on_edit_xml_declaration)
    ...
    def _populate_tree_recursive(self, parent, data):
        if isinstance(data, dict):
            for key, value in data.items():
                if key.startswith('@'):
                    ...
                    if isinstance(value, list):
                        ...
                        display_value = self.controller.format_value(value)
            ...
            display_key = "Value"
            display_value = self.controller.format_value(data)
            tag = self.controller.get_tag(data)
